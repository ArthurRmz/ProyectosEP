 ...
 *
 */
public class MenuUsuariosMostrarBuscar implements ActionListener{
	 ...
	 * @param tabla
	 */
	public MenuUsuariosMostrarBuscar(JTextField entrada, JComboBox<String> opciones,
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (!filtro.isEmpty() && !opcion.isEmpty()) {
			...
			ResultSet rs = bd.query("SELECT * FROM (SELECT ID, NOMBRE, CLAVE FROM USUARIOS WHERE ESTATUS = 1 AND LOWER("+opcion+") LIKE ? ORDER BY ID DESC) WHERE ROWNUM<11",parametros);
			try {
