 ...
 *
 */
public class MenuReportesMostrarBuscar implements ActionListener{
	 ...
	 * @param tabla
	 */
	public MenuReportesMostrarBuscar(JTextField entrada, JComboBox<String> opciones,
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (!filtro.isEmpty() && !opcion.isEmpty()) {
			...
			ResultSet rs = bd.query("SELECT * FROM (SELECT ID,FECHA,HORAINI,HORAFIN,ACTIVIDADRE,RESPONSABLERE,ACTIVIDADPRO,RESPONSABLEPRO,TURNO FROM BITACORA WHERE ESTATUS = 1 AND LOWER("+opcion+") LIKE ? ORDER BY ID DESC) WHERE ROWNUM<11",parametros);
			try {
