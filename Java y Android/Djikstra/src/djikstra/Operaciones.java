...
public class Operaciones {
	...
	public void iniciarNodos(ArrayList<Nodo> nodos) {
		nodos.add(new Nodo("A", "B", 2));
		nodos.add(new Nodo("A", "C", 3));
		nodos.add(new Nodo("B", "C", 1));
//		nodos.add(new Nodo("B", "C", 5));
//		nodos.add(new Nodo("B", "F", 1));
//		nodos.add(new Nodo("B", "E", 4));
//		nodos.add(new Nodo("C", "D", 4));
//		nodos.add(new Nodo("C", "F", 7));
//		nodos.add(new Nodo("C", "G", 3));
//		nodos.add(new Nodo("D", "G", 5));
//		nodos.add(new Nodo("E", "F", 5));
//		nodos.add(new Nodo("E", "I", 7));
//		nodos.add(new Nodo("F", "G", 1));
//		nodos.add(new Nodo("F", "H", 2));
//		nodos.add(new Nodo("G", "H", 1));
//		nodos.add(new Nodo("H", "I", 1));
	}
	...
	public Tabla djkistra(Grafo grafo, String nodoInicio) {
		...
		while (!nodoInicio.equalsIgnoreCase("null")) {
			...
			ArrayList<AuxEntrada> auxEntradas = buscarNodosAdyacentes(nodos, nodoInicio, pesoNodoInicio, 0);
	...
	public String caminoMasCorto(Grafo grafo, Tabla tabla, String nodoInicio, String nodoFinal) {
		...
		int pesoIni = pesoNodoFinal;
		ArrayList<AuxEntrada> auxEntradas = buscarNodosAdyacentes(nodos, nodoFinal, pesoNodoFinal, 1);
		...
		while (!camino.get(camino.size() - 1).equalsIgnoreCase(nodoInicio)) {
			...
			auxEntradas = buscarNodosAdyacentes(nodos, nodoFinal, pesoNodoFinal, 1);
			cambiarValoresCaminoCorto(auxEntradas);
			auxiliar.clear();
		}
		String path = "";
	...
	/*
	 * for(AuxEntrada a : auxEntradas){ System.out.println("\n" + a.getNombreNodo()
	 * + " " + a.getPeso()); }
	 */
...
//                    + t.getFin() + " " + t.isVisitado());
	...
	/*
	 * Imprimir entradas debug for(EntradaTabla t : entradas){
	 * System.out.println(t.getNodo() + " " + t.getInicio() + " " + t.getFin()); }
	 */
	...
	public void actualizarEntradas(ArrayList<EntradaTabla> entradas, ArrayList<AuxEntrada> auxEntradas) {
		for (int i = 0; i < auxEntradas.size(); i++) {
			...
			for (int j = 0; j < entradas.size(); j++) {
				...
				if (entrada.getNodo().equalsIgnoreCase(nodo) && (entrada.getFin() == INF || peso < entrada.getFin())) {
					entrada.setFin(peso);
				}
			}
		}
	...
	public int getPesoNodoInicio(ArrayList<EntradaTabla> entradas, String nodoInicio) {
		for (EntradaTabla e : entradas)
			if (e.getNodo().equalsIgnoreCase(nodoInicio))
				return e.getInicio();
		return 0;
	...
	public ArrayList<AuxEntrada> buscarNodosAdyacentes(ArrayList<Nodo> nodos, String nodo, int pesoNodoInicio,
			int operacion) {
		if (operacion == 1)
		...
		for (int i = 0; i < nodos.size(); i++) {
			...
			if (nodoAux.getPrimerNodo().equalsIgnoreCase(nodo))
				auxEntradas.add(new AuxEntrada(nodoAux.getSegundoNodo(), nodoAux.getPeso() + pesoNodoInicio));
			if (nodoAux.getSegundoNodo().equalsIgnoreCase(nodo))
				auxEntradas.add(new AuxEntrada(nodoAux.getPrimerNodo(), nodoAux.getPeso() + pesoNodoInicio));
		}
		return auxEntradas;
	}

	public void setNombreNodos(ArrayList<Nodo> nodos, ArrayList<EntradaTabla> entradas) {
		for (int i = 0; i < nodos.size(); i++) {
	...
	public void setValorInicio(ArrayList<EntradaTabla> entradas, String nodoInicio) {
		for (EntradaTabla t : entradas)
	...
	public boolean existeElemento(ArrayList<EntradaTabla> entradas, String elemento) {
	...
	public void imprimirNodos(ArrayList<Nodo> nodos) {
		for (Nodo nodo : nodos) {
			System.out.println(
					"Nodo1: " + nodo.getPrimerNodo() + " peso: " + nodo.getPeso() + " nodo2: " + nodo.getSegundoNodo());
		}
	}
}
